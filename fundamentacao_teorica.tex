...
\citeonline{zhang2004optimalitybayes} afirma que Naïve Bayes é um dos algoritmos mais eficientes para \textit{machine learning} e mineração de textos. Esse algoritmo de aprendizagem supervisionado utiliza do Teorema de Bayes, porém considerando haver independência entre as \textit{features} (característica das variáveis de entrada) do conteúdo sendo classificado.

O Teorema de Bayes com features dependentes pode ser definido da seguinte maneira:

%%%Essa equação esta errada, olhar a referencia do paragrafo acima! 
\begin{equation}
